...
let vue = null, pouch = null, defaultDB = null, defaultUsername = null,
    defaultPassword = null, databases = {}, optionsDB = {};
    ...
    created() {
        ...
        let $pouch = {
            ...
            deleteUser(username, db = databases[defaultDB]) {
                return db.deleteUser(username).catch(error => {
                    return new Promise(resolve => {
                        resolve(error);
                    });
                });
            ...
            changePassword(username, password, db = databases[defaultDB]) {
                return db.changePassword(username, password).catch(error => {
                    return new Promise(resolve => {
                        resolve(error);
                    });
                });
            ...
            changeUsername(
                oldUsername, newUsername, db = databases[defaultDB]) {
                return db.changeUsername(oldUsername, newUsername)
                    .catch(error => {
                        return new Promise(resolve => {
                            resolve(error);
                        });
            ...
            signUpAdmin(
                adminUsername, adminPassword, db = databases[defaultDB]) {
                return db.signUpAdmin(adminUsername, adminPassword)
                    .catch(error => {
                        return new Promise(resolve => {
                            resolve(error);
                        });
            ...
            deleteAdmin(adminUsername, db = databases[defaultDB]) {
                return db.deleteAdmin(adminUsername).catch(error => {
                    return new Promise(resolve => {
                        resolve(error);
                    });
                });
            ...
            push(localDB, remoteDB, options = {}) {
                ...
                let rep = databases[localDB]
                              .replicate.to(databases[remoteDB], options)
                              .on('paused',
                                  err => {
                                      if (err) {
            ...
            pull(localDB, remoteDB, options = {}) {
                ...
                let rep = databases[localDB]
                              .replicate.from(databases[remoteDB], options)
                              .on('paused',
                                  err => {
                                      if (err) {
            ...
            putAttachment(db, docId, rev, attachment) {
                ...
                return databases[db].putAttachment(
                    docId, attachment.id, rev ? rev : null, attachment.data,
                    attachment.type);
            },
        ...
        Object.keys(pouchOptions).map(key => {
            ...
            vm.$watch(pouchFn, config => {
                // if the selector is now giving a value of null or undefined,
                // then return the previous liveFeed object will remain
                if (!config) {
